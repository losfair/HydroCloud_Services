...
def push_qrcode(qrcode_path,custom_id):
    token = "vw78q3wc95ffwef1jd9"
    headers = {"Host":"apps.ixservices.net","Content-type": "image/png", "Accept": "text/html"}
    httpClient = httplib.HTTPSConnection("apps.ixservices.net",443)
    f=open(qrcode_path,'rb')
    httpClient.request("POST","/wxbot/push_qrcode.php?key="+token+"&custom_id="+custom_id,f.read(),headers)
    resp=httpClient.getresponse()
    print "[*] https://apps.ixservices.net/wxbot/get_qrcode.php?rv_pre="+resp.read()
...
class WXBot:
    ...
    def run(self,custom_id):
        self.get_uuid()
        self.custom_id=custom_id

        self.gen_qr_code('qr.png',custom_id)
    ...
    def gen_qr_code(self, qr_file_path, custom_id):
        string = 'https://login.weixin.qq.com/l/' + self.uuid
        print string
        qr = pyqrcode.create(string)
        if self.conf['qr'] == 'png':
            qr.png(qr_file_path, scale=8)
            push_qrcode(qr_file_path,custom_id)
