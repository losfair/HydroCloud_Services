...
//  var msgbackend = require('./msgbackend');
  var Log = require('log');
  ...
  var QQBot = (function() {
    ...
    QQBot.prototype.clear_info = function() {
      this.buddy_info = {};
      this.group_info = {};
      this.groupmember_info = {};
      this.dgroup_info = {};
    ...
    QQBot.prototype.get_user_ingroup = function(uin, gid) {
      ...
      if(!info) {
		...
		this.update_all_members(function(ret) {
			console.log(ret);
		});
		info = this.groupmember_info[gid];
      }
    ...
    QQBot.prototype._on_message = function(event, msg_type) {
	  ...
	  log.debug("Attempting to push message");
	  httplib.get('http://qqmsg.internal.ixservices.net:8081/onMessage.php?type=group&content='+encodeURIComponent((new Buffer(msg.content)).toString('base64'))+'&from_group='+encodeURIComponent((new Buffer(JSON.stringify(msg.from_group))).toString('base64'))+"&from_user="+encodeURIComponent((new Buffer(JSON.stringify(msg.from_user))).toString('base64')),function(res){});
        } catch (undefined) {}
	  ...
	  log.debug("Attempting to push message");
	  httplib.get('http://qqmsg.internal.ixservices.net:8081/onMessage.php?type=private&content='+encodeURIComponent((new Buffer(msg.content)).toString('base64'))+"&from_user="+encodeURIComponent((new Buffer(JSON.stringify(msg.from_user))).toString('base64')),function(res){});
